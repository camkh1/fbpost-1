...
class Hybrid_Provider_Adapter
{
	...
	*/
	function login()
	{
        ...
        $urlDone = $HYBRID_AUTH_URL_BASE . ( strpos( $HYBRID_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";
        $changeToDone = str_replace('http://tobteng.com/index.phphttp://','http://',$urlDone);
		$this->params["login_done"]  = $changeToDone;
        ...
        $changeTo = str_replace('http://tobteng.com/index.phphttp://','http://',$this->params["login_start"]);
		Hybrid_Auth::redirect($changeTo);
	}
