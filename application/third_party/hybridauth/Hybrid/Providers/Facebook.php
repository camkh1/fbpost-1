...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	//public $scope = "email, user_about_me, user_birthday, user_hometown, user_website, read_stream, offline_access, publish_stream, read_friendlists,user_groups";
	...
	*/
	function initialize() 
	{
		...
		$this->api = new Facebook( ARRAY( 'appId' => $this->config["keys"]["id"], 'secret' => $this->config["keys"]["secret"] ,'cookie' => true) ); 
	...
	*/
	...
	{ 
        parent::clearToken();
        $url = $this->getLogout(self::getCurrentUrl());
	...
	*    - me       : the user activity only  
	*/
	function getUserGroup($queries)
	{
        // POST your queries to the batch endpoint on the graph.
        try {
            $batchResponse = $this->api->api('?batch=' . json_encode($queries), 'POST');
        } catch (Exception $o) {
            throw new Exception( "User activity stream request failed! {$this->providerId} returned an error: $e" );
        }
    ...
    function Authorisingurl() {
        echo 11111111;die;
        
    }
    
    
	function getLogout($current)
	{
        // POST your queries to the batch endpoint on the graph.
        try {
            $logoutUrl = $this->api->getLogoutUrl(array(
        'next' => $current));
            $this->api->destroySession();
        } catch (Exception $o) {
            throw new Exception( "User activity stream request failed! {$this->providerId} returned an error: $e" );
