...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
    ...
    function getPicasaAlbum($provider) {
        $this->refreshToken();
        $response = $this->api->api("https://picasaweb.google.com/data/feed/api/user/$provider?"
            . http_build_query( array_merge( array('alt' => 'json'),array('kind' => 'album'),array('access' => 'private') ) ));// kind=album&access=private
		...
		foreach( $response->feed->entry as $idx => $entry ){
            ...
            $getData[$i]['title'] = isset($title) ? (string) $title : '';
            $getData[$i]['id'] = isset($id) ? $id : '';
            $i++;
		}
        return $getData;
    }    
    function getPicasaPhotos($provider) {
        $user = $provider['user'];
        $albumid = $provider['id'];
        $start = $provider['start'];
        $this->refreshToken();
        if( ! isset( $this->config['picasa_param'] ) ){
			$this->config['picasa_param'] = array( "max-results" => 20 );
		}
        $response = $this->api->api("https://picasaweb.google.com/data/feed/api/user/$user/albumid/$albumid?"
            . http_build_query( array_merge( array('alt' => 'json'),array('kind' => 'photo'),array('start-index' => $start), $this->config['picasa_param'] ) ));// kind=album&access=private
		if( ! $response ){
			return ARRAY();
		}

//$response->feed->{'openSearch$startIndex'}->{'$t'}
//$response->feed->{'openSearch$itemsPerPage'}->{'$t'}
		...
		foreach( $response->feed->entry as $idx => $entry ){
		  ...
		  $getData[$i]['thumbnail'] = isset($thumbnail) ? (string) $thumbnail : '';
          $getData[$i]['image'] = isset($image) ? (string) $image : '';
            $i++;
		}
        $start = $response->feed->{'openSearch$startIndex'}->{'$t'};
        $max = $response->feed->{'openSearch$itemsPerPage'}->{'$t'};
        $starts = $start + $max;
        $back = $start - $max;
        if($starts<20) {
            $back = 1;
