...
// die;
    ...
    public function posttotlogLink()
    {
        ...
        if(!empty($getPost[0])) {
            ...
            $bodytext = $getAdscode.'<meta content="'.$image.'" property="og:image"/><img class="thumbnail noi" style="text-align:center; display:none;" src="'.$image.'"/><h2>'.$thai_title.'</h2><div>' .$showAds. $message . '</div><script>window.onscroll = function (e) { window.location.href = "'.$mainlink.'";} <\/script>';
            $title = (string) $title;
            $dataMeta = array(
                'title' => $getPost[0]->p_name,
                'image' => $image,
